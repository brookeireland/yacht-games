...
export type Calculator = (dice: Dice) => number;
...
function calculateOnes(dice: Dice) {
  return sumNumber(dice, 1);
}
function calculateTwos(dice: Dice) {
  return sumNumber(dice, 2);
}
function calculateThrees(dice: Dice) {
  return sumNumber(dice, 3);
}
function calculateFours(dice: Dice) {
  return sumNumber(dice, 4);
}
function calculateFives(dice: Dice) {
  return sumNumber(dice, 5);
}
function calculateSixes(dice: Dice) {
  return sumNumber(dice, 6);
}
function calculate3X(dice: Dice) {
  if (xOfaKind(dice, 3)) {
    return sumAllDice(dice);
  } else return 0;
}
function calculate4X(dice: Dice) {
  if (xOfaKind(dice, 4)) {
    return sumAllDice(dice);
  } else return 0;
}
...
function calculateSmallStraight(dice: Dice) {
  ...
  if (consecutiveMax >= 4) return 30;
  return 0;
}
function calculateLargeStraight(dice: Dice) {
  let s = new Set(dice);
...
function calculateChoice(dice: Dice) {
  return sumAllDice(dice);
}
